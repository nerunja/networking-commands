...
# Add a VLAN sub‑interface and a VM tap to the bridge
...
### 1) Single‑host isolation (local VLAN + bridge)

```
VMs/containers (veth/tap) ──┐
                            ├── br0 ── eno1.20 (VLAN 20) ── eno1 ──(to switch)
Host services on VLAN 20 ───┘
```

- VLAN used **locally** on host; same VLAN can also be extended upstream if the switch port is a trunk that allows VLAN 20.

### 2) Multi‑host, same VLAN across hosts

```
Host A: vnetA ─┐              ┌─ vnetB :Host B
...
- VMs/containers on VLAN 20 across hosts share the **same L2 broadcast domain**.
